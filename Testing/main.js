...
for (let x = 0; x <= TamañoArray; x += 1) {
  ...
  // comparador
  if (indice1 < indice2) {
    console.log(`este valor [${myArray[x]}] es menor que el valor [${myArray[x + 1]}], NO MOVER`)
  } if (myArray[x] > myArray[x + 1]) {
    // almacen temporal
    console.log(`este valor [${myArray[x]}] es mayor que el valor [${myArray[x + 1]}], SI MOVER`)
